...
def drop_identifier_like_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Drop obvious row identifiers that are not behavioral security evidence."""
    identifiers = [
...
def add_safe_derived_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Add conservative teaching features only when required columns exist.

    These features are examples for experimentation, not claims that they
    are optimal production detection features.
    """
    out = df.copy()
